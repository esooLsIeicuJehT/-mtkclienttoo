#!/usr/bin/env bash
# MTK Client 2.0 — Fedora/Linux Installer
...
install_fedora() {
    ...
    sudo dnf install -y \
        ...
        android-tools \
        usbutils
    pip3 install --user pyusb requests cryptography Pillow colorlog
}
...
install_debian() {
    ...
    sudo apt install -y \
        ...
        android-tools-adb \
        usbutils
    pip3 install --user requests cryptography Pillow colorlog
}
...
case "$DISTRO" in
    ...
    *)
        if echo "$DISTRO_LIKE" | grep -q "fedora\|rhel"; then
            ...
            pip3 install --user pyusb pyserial PyQt5 requests cryptography Pillow colorlog
        fi
        ;;
esac
